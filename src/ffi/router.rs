...
pub unsafe extern "C" fn router_enable_prefilter(router: &mut Router<&Schema>, field: *const u8) {
    use std::io::prelude::*;
    let mut f = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/prefilter_addrs.txt").unwrap();
    writeln!(f, "{field:p}").unwrap();
    drop(f);
    let field = ffi::CStr::from_ptr(field as *const c_char).to_str().unwrap();
